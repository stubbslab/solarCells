...
def readInSCandPDDataFile(data_file, data_dir, SC_neg_current, PD_neg_current, ignore_str = 'DELETED', indeces_to_include = 'all'):
    ...
    angles = [int(row[1])  for row in data]

    PD_ill_times = [[ ( float(elem)) for elem in can.recursiveStrToListOfLists(row[4]) if elem != ignore_str] for row in data]
    ...
    PD_dk_times = [[ ( float(elem)) for elem in can.recursiveStrToListOfLists(row[7]) if elem != ignore_str] for row in data]
    ...
    SC_ill_times = [[ (float(elem)) for elem in can.recursiveStrToListOfLists(row[10]) if elem != ignore_str] for row in data]
    ...
    SC_dk_times = [[ (float(elem)) for elem in can.recursiveStrToListOfLists(row[13]) if elem != ignore_str] for row in data]
    ...
    arrs = [wavelengths, angles, SC_ill_times, SC_illum, SC_illum_bv, SC_dk_times, SC_dark, SC_dark_bv, PD_ill_times, PD_illum, PD_illum_bv, PD_dk_times, PD_dark, PD_dark_bv, SC_temps]
...
if __name__ == "__main__":
    ...
    stems = ['SCFX_Wave50nm_400_1000_30deg_NPD50_NSC50_']
    #suffixes = ['_1', '_1', '_MultiDay']
    suffixes = ['']
    #extra_save_suffixes = ['', '', '']
    extra_save_suffixes = ['_QEVsWavelength']
    date_strs = ['20210929']
    extra_title_str = 'Cell ID HG, New Stable Light Source'
    ...
    for i in range(len(data_files)):
        ...
        wavelengths, angles, SC_illum_times, SC_illum, SC_illum_bv, SC_dark_times, SC_dark, SC_dark_bv, PD_illum_times, PD_illum, PD_illum_bv, PD_dark_times, PD_dark, PD_dark_bv, SC_temps = readInSCandPDDataFile(data_file, data_dir, SC_neg_current, PD_neg_current, ignore_str = deleted_element_id, indeces_to_include = data_file_indeces_to_include)
        wavelengths = [float(wave) + wavelength_correction for wave in wavelengths]
        angles = [angle - zero_angles[i] for angle in angles]
        ...
        angles = [angles[j] for j in range(len(angles)) if not (j in bad_data_indeces[i]) ]
        ...
        stat_arrs = [PD_x_vals, SC_x_vals, SC_diff, SC_diff_std, PD_diff, PD_diff_std, ratios, ratio_errs]
        stats_by_angles = {angle:[[] for i in range(len(stat_arrs))] for angle in np.unique(angles)}
        #angle_colors = cm.rainbow(np.linspace(0, 1.0, len(np.unique(angles))))
        #angle_colors = [ [color] for color in angle_colors]
        angle_colors = ['violet','indigo', 'blue','green','yellow','orange','red' ]
        print ('len(np.unique(angles) = ' + str(len(np.unique(angles))))
        colors_by_angles = { np.unique(angles)[k]:angle_colors[k] for k in range(len(np.unique(angles))) }

        for j in range(len(angles)):
            angle = angles[j]
            for k in range(len(stat_arrs)):
                stats_by_angles[angle][k] =  stats_by_angles[angle][k] + [stat_arrs[k][j]]
        ...
        for j in range(len(np.unique(angles))):
            angle = np.unique(angles)[j]
            print ('angle = ' + str(angle))
            angle_color = colors_by_angles[angle]
            SC_plots = SC_plots + [axarr_QE[0,0].scatter(stats_by_angles[angle][1], stats_by_angles[angle][2], c=angle_color, marker = '.')]
            #axarr_QE[0,0].plot(SC_x_vals, SC_diff, c=SC_color, marker = '.')
            axarr_QE[0,0].errorbar(stats_by_angles[angle][1], stats_by_angles[angle][2], yerr = stats_by_angles[angle][3], color=angle_color, fmt = 'none')
            PD_plots = PD_plots + [axarr_QE[0,0].scatter(stats_by_angles[angle][0], stats_by_angles[angle][4], c=angle_color, marker = 'x')]
            #axarr_QE[0,0].plot(PD_x_vals, PD_diff, c=PD_color, marker = '.')
            axarr_QE[0,0].errorbar(stats_by_angles[angle][0], stats_by_angles[angle][4], yerr = stats_by_angles[angle][5], color=angle_color, fmt = 'none')
            lp_in_first = axarr_QE[0,0].axvline(lp_filter_wavelength, color = indicator_color, alpha = 0.5, linestyle = 'dashed')
            ratio_plots = ratio_plots + [axarr_QE[0,1].scatter(stats_by_angles[angle][1], stats_by_angles[angle][6], c = angle_color, marker = '.')]
            axarr_QE[0,1].errorbar(stats_by_angles[angle][1], stats_by_angles[angle][6], yerr = stats_by_angles[angle][7], fmt = 'none', color = angle_color)
            lp_in_second = axarr_QE[0,1].axvline(lp_filter_wavelength, color = angle_color, alpha = 0.5, linestyle = 'dashed')
            ...
            #axarr_QE[1,1].errorbar(stats_by_angles[angle][1], PD_QE_interp(true_wavelengths) * np.array(stats_by_angles[angle][6]), yerr = PD_QE_interp(true_wavelengths) * np.array(stats_by_angles[angle][7]), fmt = 'none', color = SC_color)
            #axarr_QE[1,1].scatter(qe_spline_wavelengths, PD_QE_interp(qe_spline_wavelengths) * qe_spline_ratios, marker = 'x', c ='k')
            zoomed_SC_QE_plots = zoomed_SC_QE_plots + [axarr_QE[1,1].scatter(stats_by_angles[angle][1], PD_QE_interp(stats_by_angles[angle][1]) * np.array(stats_by_angles[angle][6]), c = angle_color, marker = '.')]
            full_SC_QE_plots = full_SC_QE_plots + [axarr_QE[1,0].scatter(stats_by_angles[angle][1], PD_QE_interp(stats_by_angles[angle][1]) * np.array(stats_by_angles[angle][6]), c = angle_color, marker = '.')]
        #axarr_QE[0,0].set_xlim(x_lims)
        #axarr_QE[0,1].set_xlim(x_lims)
        #axarr_QE[1,0].set_xlim(x_lims)
        #axarr_QE[1,1].set_xlim(x_lims)
        PD_QE_plots = PD_QE_plots + [axarr_QE[1,0].scatter(stats_by_angles[angle][1], PD_QE_interp(stats_by_angles[angle][1]), color = PD_color, marker = 'x')]
    ...
    print ('len(SC_plots) = ' + str(len(SC_plots)))
    print ('insert_longpass = ' + str(insert_longpass))

    if insert_longpass:
        axarr_QE[0,0].legend(PD_plots + SC_plots + [lp_in_first], [r'PD $I$ @ $\phi =$ ' + str(angle) + ' deg' for angle in unique_angles] +  [r'SC $I$ @ $\phi =$ ' + str(angle) + ' deg' for angle in unique_angles] + ['>600nm Longpass Inserted'] , fontsize = 8, ncol = 2)
        axarr_QE[0,1].legend(ratio_plots + [lp_in_second], [r'SC/PD $I$ ratio @ ' + str(angle) + ' deg' for angle in unique_angles] + ['>600nm Longpass Inserted'], fontsize = 8, ncol = 2)
#axarr_QE[2].legend([SC_QE_plots[0], PD_QE_plots[0], lp_in_third], ['Inferred Solar Cell QE', 'NIST Photodiode QE', 'Longpass Inserted'])
        axarr_QE[1,0].legend([PD_QE_plots[-1]] + full_SC_QE_plots, ['NIST Photodiode QE (NOT our data)'] + [r'Inferred SC QE @ $\phi =$ ' + str(angle) + ' deg' for angle in unique_angles], fontsize = 8, ncol = 2)
        axarr_QE[1,1].legend(zoomed_SC_QE_plots + [lp_in_forth], ['Inferred SC QE @ $\phi =$ ' + str(angle) + ' deg' for angle in unique_angles] + ['>600nm Longpass Inserted'], fontsize = 8, ncol = 2)
    else:
        axarr_QE[0,0].legend(PD_plots + SC_plots,  ['PD photocurrent: ' + cell_id for cell_id in cell_ids] + ['SC photocurrent: ' + cell_id for cell_id in cell_ids]  , fontsize = 4)
        axarr_QE[0,1].legend(ratio_plots  , ['SC/PD photocurrent ratio: ' + cell_id for cell_id in cell_ids], fontsize = 4)
#axarr_QE[2].legend([SC_QE_plots[0], PD_QE_plots[0], lp_in_third], ['Inferred Solar Cell QE', 'NIST Photodiode QE', 'Longpass Inserted'])
        axarr_QE[1,0].legend([PD_QE_plots[-1]] + full_SC_QE_plots, ['NIST PD QE (NOT our data)'] + ['Inferred SC QE: ' + cell_id for cell_id in cell_ids], fontsize = 4)
        axarr_QE[1,1].legend(zoomed_SC_QE_plots , ['Inferred SC QE: ' + cell_id for cell_id in cell_ids], fontsize = 4)
